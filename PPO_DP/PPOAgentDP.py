...
from GetFairness import GetMetrics # Current rewrads
from FairnessMetrics import FairnessMetrics # State Values
...
"""
Update method for PPO + demographic parity
"""

class PPOAgentDP(PPOAgent):
    def __init__(self, *args, alpha, beta, value_loss_coef=1.0, config = None, **kwargs):
        ...
        self.fairness_metrics_rewards = GetMetrics() # Current rewrads
        self.fairness_metrics_state = FairnessMetrics() # State Values
    ...
    def update(self, memory_non_sensitive, memory_sensitive):
        loss_non_sensitive, ppo_loss_non_sensitive, scaled_penalty_non_sensitive, lambda_dp_non_sensitive= self._update_memory(memory_non_sensitive, memory_sensitive)
        loss_sensitive, ppo_loss_sensitive, scaled_penalty_sensitive, lambda_dp_sensitive = self._update_memory(memory_sensitive, memory_non_sensitive)

        combined_loss = loss_non_sensitive+ loss_sensitive

        return combined_loss, loss_non_sensitive, loss_sensitive, ppo_loss_non_sensitive, ppo_loss_sensitive, scaled_penalty_non_sensitive, scaled_penalty_sensitive, lambda_dp_non_sensitive, lambda_dp_sensitive
