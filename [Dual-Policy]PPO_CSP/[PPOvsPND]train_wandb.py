...
from PPOAgentCSP import PPOAgentCSP, Memory
...
def train_ppo(ppo_agent_non_sensitive, ppo_agent_sensitive, config, render, verbose=False):
    ...
    lambda_csp_non_sensitive_list = []
    penalties_sensitive_list = []
    lambda_csp_sensitive_list = []
    ...
    for episode in tqdm(range(config["num_episodes"])):
        ...
        for t in range(config["max_timesteps"]):
            ...
            # Update non_sensitive agent's memory and log rewards
            non_sensitive_index = 0
            for i, r in enumerate(rewards):
                if state[i][2] == 0:  # non_sensitive agent
                    r = non_sensitive_rewards[non_sensitive_index]  # Use normalized non_sensitive reward
                    ...
                    # Log rewards by preference for non_sensitive agents
                    ppo_agent_non_sensitive.log_rewards_by_preference([r], [state[i]], 'non_sensitive')
                    
                    episode_reward_non_sensitive+= r
                    non_sensitive_index += 1

            # Update sensitive agent's memory and log rewards
            sensitive_index = 0
            for i, r in enumerate(rewards):
                if state[i][2] == 1:  # Sensitive agent
                    r = sensitive_rewards[sensitive_index]  # Use normalized sensitive reward
                    ...
                    # Log rewards by preference for sensitive agents
                    ppo_agent_sensitive.log_rewards_by_preference([r], [state[i]], 'sensitive')
                    
                    episode_reward_sensitive += r
                    sensitive_index += 1
        ...
        combined_loss, loss_non_sensitive, _, ppo_loss_non_sensitive, _, scaled_penalty_non_sensitive, _, lambda_csp_non_sensitive, _  = ppo_agent_non_sensitive.update(memory_non_sensitive, memory_sensitive)
        combined_loss, _, loss_sensitive, _, ppo_loss_sensitive, _, scaled_penalty_sensitive, _, lambda_csp_sensitive = ppo_agent_sensitive.update(memory_sensitive, memory_non_sensitive)   
        ...
        lambda_csp_non_sensitive_list.append(lambda_csp_non_sensitive.clone().detach().to(config["device"]))
        penalties_sensitive_list.append(scaled_penalty_sensitive.clone().detach().to(config["device"]))
        lambda_csp_sensitive_list.append(lambda_csp_sensitive.clone().detach().to(config["device"]))
        ...
        if len(cumulative_rewards_non_sensitive) >= rolling_window_size:
            ...
            rolling_avg_lambda_csp_non_sensitive= torch.mean(torch.stack(lambda_csp_non_sensitive_list[-rolling_window_size:]))
            rolling_avg_penalties_sensitive = torch.mean(torch.stack(penalties_sensitive_list[-rolling_window_size:]))
            rolling_avg_lambda_csp_sensitive = torch.mean(torch.stack(lambda_csp_sensitive_list[-rolling_window_size:]))
            ...
            wandb.log({
                ...
                "Rolling CSP Penalty (Non Sensitive)": rolling_avg_penalty_non_sensitive.item(),
                "Rolling Lambda (Non Sensitive)": rolling_avg_lambda_csp_non_sensitive.item(),
                "Rolling CSP Penalty (Sensitive)": rolling_avg_penalties_sensitive.item(),
                "Rolling Lambda (Sensitive)": rolling_avg_lambda_csp_sensitive.item(),
...
if __name__ == '__main__':
    ...
    weights_dir = f"[alpha={config['alpha']},beta={config['beta']}]policy-weights-csp"
